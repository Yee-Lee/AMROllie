...
1. **[已完成]** `ollie_microros.service`：負責透過 Docker 啟動 micro-ROS agent，橋接下位機 ESP32。
2. **[待完成]** `ollie_description.service`：負責發布機器人 URDF 模型與 TF 座標轉換。
3. **[待完成]** `ollie_lidar.service`：負責啟動 LD19 光達驅動。
...
# 啟動 micro-ROS agent，使用 host 網路與實體 serial port
ExecStart=/usr/bin/docker run --name microros_agent --rm --net=host --privileged -v /dev:/dev -e ROS_DOMAIN_ID=30 microros/micro-ros-agent:humble serial --dev /dev/serial0 -b 460800
# 服務停止時優雅關閉容器
...
### 步驟 2：部署 Ollie Description 機器人狀態發布 (待完成)

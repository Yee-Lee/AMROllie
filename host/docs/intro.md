...
> **💡 USB 裝置綁定**：由於 ESP32 與 LiDAR 皆依賴 USB 序列埠，為了避免重啟或插拔導致 `/dev/ttyUSB*` 順序跳動錯亂，系統透過 `udev` 規則將 ESP32 固定映射為 `/dev/ollie_core`，而 LiDAR 固定映射為 `/dev/ollie-lidar`。
...
- **實體層**：RPi3B 端透過 USB 轉 TTL 模組與 ESP32 進行 UART 序列通訊 (TX/RX 交叉接線並共地)，鮑率設定為 `115200`。

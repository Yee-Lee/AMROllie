...
本指南說明如何在 Raspberry Pi 5B (Ubuntu 24.04 / ROS 2 Jazzy) 上建立 `rosbridge_suite`，並透過網頁端的手機虛擬搖桿，實現遠端遙控 Ollie AMR。

## 系統架構簡介
* **前端介面 (手機/PC)**：透過瀏覽器開啟 HTML 網頁，使用虛擬搖桿產生控制指令。
* **通訊橋樑 (WebSocket)**：透過 `roslib.js` 將前端指令打包成 JSON，發送至 RPi。
* **ROS 2 後端 (RPi 5B)**：`rosbridge_server` 接收 JSON 並轉換為 ROS 2 標準的 `geometry_msgs/msg/Twist` 訊息，發布至 `/cmd_vel` Topic。
...
在 RPi 5B 的終端機中執行以下指令，安裝 WebSocket 通訊套件：
```bash
sudo apt update
...
```bash
mkdir -p ~/workspace/AMROllie/host_2404/ros2_ws/web_controller
cd ~/workspace/AMROllie/host_2404/ros2_ws/web_controller
...
為了讓系統順利運作，我們需要在 RPi 5B 上開啟 **三個不同的終端機 (Terminal)**。
...
### 1. 取得 RPi 5B 的區網 IP
如果你不知道 RPi 5B 的 IP 位址，可以在 RPi 上輸入 `ip a` 或 `ifconfig` 查詢（例如：`192.168.1.100`）。

### 2. 開啟手機或 PC 瀏覽器
確認你的控制裝置（手機或筆電）與 RPi 5B 連接在**同一個 Wi-Fi 網路**下。
在瀏覽器網址列輸入：
```text
http://<RPi 5B的IP位址>:8000
```
網頁載入後，你應該會看到暗黑風格的 OLLIE DASHBOARD。

### 3. 驗證資料流 (Echo Topic)
在 RPi 5B 上開啟 **[終端機 3]**，輸入以下指令來監看速度控制訊號：
```bash

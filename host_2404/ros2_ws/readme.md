...
本目錄 (`host_2404/ros2_ws`) 為 Ollie 上位機（Raspberry Pi 5B 或開發用 PC）的 ROS 2 工作區 (Workspace)。裡面存放了 Ollie 專屬的 ROS 2 套件，包含機器人模型描述 (`ollie_description`) 與光達驅動 (`ldlidar`) 等。
...
3. **分散式架構與視覺化除錯**
   ROS 2 天生支援區域網路內的多機通訊 (透過 DDS)。你可以在沒有螢幕的樹莓派上執行核心程式，然後在自己舒適的 Mac/PC 上開啟 RViz2，透過網路即時監看 Ollie 的 3D 姿態、光達點雲與感測器數據。
...
當你完成 ROS 2 安裝後，就可以來編譯本目錄 (`host_2404/ros2_ws`) 中的原始碼了。

1. 進入工作區目錄：
   ```bash
   cd ~/Workspace/AMROllie/host_2404/ros2_ws
   ```
2. 安裝套件相依性 (rosdep)：
   ```bash

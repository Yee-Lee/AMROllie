...
if [ -f "$GOV_FILE" ]; then
    ...
    if [ "$GOV" = "performance" ]; then
        echo "✅ 正確：CPU 運行於 $GOV 模式 (⚠️ RPi 5B 請確認已安裝散熱器)"
    else
        echo "⚠️ 提醒：CPU 運行於 $GOV 模式。若有安裝散熱器，建議設為 performance；若無，請保持現狀避免過熱降頻。"
    fi
else
...
check_service_masked() {
    STATUS=$(systemctl is-enabled "$1" 2>/dev/null || echo "not-found")
    if [ "$STATUS" = "masked" ] || [ "$STATUS" = "disabled" ] || [ -z "$STATUS" ] || [ "$STATUS" = "not-found" ] || echo "$STATUS" | grep -q "No such file"; then
        echo "✅ $1 已被正確遮蔽或移除"
    else
...
check_service_masked "unattended-upgrades.service"
check_service_masked "multipathd.service"
